...
echo "QEMU_BOARD_DIR    : ${QEMU_BOARD_DIR}"
echo "DEFCONFIG_NAME    : ${DEFCONFIG_NAME}"
echo "README_FILES      : ${README_FILES}"
echo "START_QEMU_SCRIPT : ${START_QEMU_SCRIPT}"
echo "DTB               : ${DTB}"

if [[ "${DEFCONFIG_NAME}" =~ ^"qemu_*" ]]; then
    echo "Not a Qemu defconfig, can't test." >&2
    exit 0
fi
...
if [ -z "${QEMU_CMD_LINE}" ]; then
    echo "No Qemu cmd line found, can't test." >&2
    exit 0
fi
